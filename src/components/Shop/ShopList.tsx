...
export function ShopList() {
  ...
  const filteredShops = useMemo(() => {
    if (!searchKeyword.trim()) return shops;
    const kw = searchKeyword.toLowerCase();
    return shops.filter((s) => {
      if (searchField) {
        const v = (s as any)[searchField];
  ...
  }, [shops, searchKeyword, searchField]);
  ...
  return (
    <Box>
      ...
      <SearchBar
        ...
        onClear={handleClear}
      />
      <DataTable columns={columns} data={pagedShops} loading={loading} error={error} onRefresh={load}
        onEdit={(row) => { setEditingShop(row); setFormOpen(true); }}
        onDelete={(row) => setDeleteTarget(row)}
        pagination={{
          page, pageSize, total: filteredShops.length,
