...
export function AdminForm({ open, user, onClose, onSuccess }: Props) {
  ...
  const handleSubmit = async () => {
    if (!account.trim()) {
      enqueueSnackbar("账号不能为空", { variant: "error" });
      return;
    }
    if (!isEdit && !password) {
      enqueueSnackbar("密码不能为空", { variant: "error" });
      return;
    }
    if (password && password.length < 8) {
      enqueueSnackbar("密码至少8位", { variant: "error" });
      return;
    }
      ...
      <DialogContent>
        ...
        <FormControl fullWidth margin="normal">
          ...
          <Select
            value={isSuper}
            label="管理员权限"
            onChange={(e) => setIsSuper(Number(e.target.value))}
          >
